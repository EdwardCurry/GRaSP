...
The current codebase implements a **true sparse, prior-guided neighbor attention**:
...
- **`data_augmentation.py`**
  - Rebuilds the experiment **without data leakage**:
...
- **`main.py`**  (TRAINING ONLY)
  - Trains the joint model with **true sparse neighbor attention** guided by knowledge priors.
  - Saves:
...
### Step 1: Data Augmentation (Leakage-Free)
...
Train with true sparse neighbor attention:
...
* The cross-attention implementation is **truly sparse** and only computes attention on prior-allowed edges.
